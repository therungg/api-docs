...
Here are the TypeScript Interfaces I use for the events. Each one of them contains all the fields in `MarathonEvent`, where `data` is overridden, and to be specified seperately for each event.
...
```
The following events will only be available when they fulfill a certain condition. They will appear on the left side of the screen.
```typescript
